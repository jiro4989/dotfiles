...
readonly SCRIPT_USERNAME=jiro4989
readonly SCRIPT_USEREMAIL=jiroron666@gmail.com
...
# Neovim
curl -sSfL https://github.com/neovim/neovim/releases/download/nightly/nvim.appimage > /tmp/nvim
sudo install -o root -g root -m 0755 /tmp/nvim /usr/local/bin/nvim
...
# shfmt
curl -sSfL https://github.com/mvdan/sh/releases/download/v3.0.1/shfmt_v3.0.1_linux_amd64 > /tmp/shfmt
...
# Git
git config --global user.email "$SCRIPT_USEREMAIL"
git config --global user.name "$SCRIPT_USERNAME"
...
# docker
sudo usermod -a -G docker "$SCRIPT_USERNAME"

# change shell
sudo chsh -s $(which tmux) "$SCRIPT_USERNAME"

# gh
curl -sSfL https://github.com/cli/cli/releases/download/v0.10.0/gh_0.10.0_linux_amd64.deb > /tmp/gh_0.10.0_linux_amd64.deb
sudo apt install -y /tmp/gh_*_linux_amd64.deb
